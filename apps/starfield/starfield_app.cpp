...
void StarfieldApp::loop() {
    ...
    if (_overlay[0][0]) {
        uint16_t cyan = rgb565(0, 255, 208);
        uint16_t dim = rgb565(0, 128, 104);
        int y_start = _h / 2 - 20;
        drawString(4, y_start,      _overlay[0], cyan);
        drawString(4, y_start + 10, _overlay[1], dim);
        drawString(4, y_start + 20, _overlay[2], dim);
    }
