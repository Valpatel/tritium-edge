...
bool init(esp_lcd_panel_handle_t panel, int width, int height) {
    ...
    // Initialize screensaver (inactivity-triggered starfield overlay)
    shell_screensaver::init(width, height);
...
void setBatteryStatus(int percent, bool charging) {
    ...
    if (percent < 0) {
        // No battery — USB/mains powered (e.g. 43C-BOX)
    ...
    if (s_batt_pct_label) {
        if (percent < 0) {
            lv_label_set_text(s_batt_pct_label, "USB");
            lv_obj_set_style_text_color(s_batt_pct_label, T_GREEN, 0);
        } else {
            char buf[8];
