...
// Elongated octahedron — horizontal axis for wide screens (800x480).
// The crystal is wider than tall so it doesn't clip on landscape displays.
static const Vec3 VERTS[6] = {
    { 0.0f,  1.0f,  0.0f},  // top
    { 0.0f, -1.0f,  0.0f},  // bottom
    { 1.6f,  0.0f,  0.0f},  // right
    {-1.6f,  0.0f,  0.0f},  // left
...
void showLogo(const char* version) {
    ...
    // Show animated "INITIALIZING" while services are starting
    int init_x = _margin_x;
    int init_y = _line_y;
    draw_string(init_x, init_y, "INITIALIZING ", COL_DIM_CYAN, _scale);
    int dots_x = init_x + measure("INITIALIZING ", _scale);
    push_full();

    // Animate dots while waiting for showService calls
    for (int d = 0; d < 6; d++) {
        char dot_str[4] = {'.', '\0'};
        draw_string(dots_x + d * 6 * _scale, init_y, dot_str, COL_CYAN, _scale);
        push_region(init_y, init_y + _line_height);
        delay(80);
    }

    // Clear the initializing line — services will overwrite below
    clear_region(init_y, init_y + _line_height + 2);
    push_region(init_y, init_y + _line_height + 2);
}
...
void showService(const char* name, const char* status, const char* detail) {
    ...
    float wobble_x = sinf(_crystal_angle * 0.4f) * 0.15f;
    clear_region(0, _crystal_clip_y);
    draw_stars();
    draw_crystal(_crystal_angle, wobble_x, 1.0f, 1.0f);
    draw_particles(_crystal_angle, wobble_x, 1.0f);
    advance_particles(0.035f * 3.0f);
    push_region(0, _crystal_clip_y);
...
void showReady() {
    ...
    // Brief hold so the SYSTEM READY message is visible
    delay(800);
}
